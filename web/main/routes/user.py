...
-----------------------------------------------------------------------------
"""
@user.route('/sensors/check/<int:id>')
@login_required
@admin_required
def check_sensor(id):
...
def main_sensors():
    ...
    if filters.validate():
        if filters.name.data != None:
            data["name"] = filters.name.data
        if filters.status.data != None:
            data["status"] = filters.status.data
        if filters.active.data != None:
            data["active"] = filters.active.data
        if filters.user_email.data != None:
